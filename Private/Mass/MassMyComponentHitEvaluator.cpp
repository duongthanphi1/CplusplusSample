...
bool FMassMyComponentHitEvaluator::Link(FStateTreeLinker& Linker)
{
...
void FMassMyComponentHitEvaluator::Tick(FStateTreeExecutionContext &Context, const float DeltaTime) const
{
	...
	if (HitResult != nullptr)
	{
		...
		if (TimeSinceHit < HitEventDuration)
		{
			...
			//UE_LOG(LogTemp,Error,TEXT("Got Hit by %s"),*static_cast<FMassStateTreeExecutionContext&>(Context).GetEntity().DebugGetDescription() );
		}
	}
}
